...
class StudentQuizAttemptViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            enrollment = Enrollment.objects.get(student=self.request.user, course=quiz.course)
        except Enrollment.DoesNotExist:
            raise PermissionDenied("You must be enrolled in the course to take this quiz")

        existing_attempts = StudentQuizAttempt.objects.filter(enrollment=enrollment, quiz=quiz).count()

        if existing_attempts >= quiz.max_attempts:
            raise ValidationError(f"Maximum attempts ({quiz.max_attempts}) reached for this quiz")

        serializer.save(enrollment=enrollment, attempt_number=existing_attempts + 1, score=0, passed=False, answers={})
    ...
    def submit(self, request, pk=None):
        ...
        for question in questions:
            ...
            if question.question_type == "multiple_choice":
                try:
                    answer = QuizAnswer.objects.get(id=submitted_answer, question=question)
                ...
                correct_answer = QuizAnswer.objects.filter(question=question, is_correct=True).first()
